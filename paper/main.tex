...
% Show line numbers during review (Elsevier requires this for the review class).
\modulolinenumbers[5]
...
We accompany the benchmark with four semantics-preserving adversarial
mutators (dead-code injection, string splitting, identifier rename,
wrapper extraction), each targeting a known detection shortcut. By
measuring per-tool F1 drop per mutator we extend CASTLE's framing from
``which tools detect this vulnerability'' to ``which transformations
break which tools.''
% TODO: insert one-line headline result once SAST and LLM evaluations
% have been run (e.g., ``LLMs lose 19~F1 points on average to wrapper
% extraction; SAST tools lose 33~F1 points to string splitting'').
All dataset artifacts, audit reports, rejection manifests, and tool
runners are released under a permissive license to support replication
and extension.
\end{abstract}
...
% Line numbers on for review class.
\linenumbers
