...
PrimeVul~\cite{TODO_primevul} demonstrated this directly: a code
...
CleanVul~\cite{TODO_cleanvul} quantified the underlying problem:
...
The closest prior work to ours is RealVuln~\cite{TODO_realvuln}, which
...
benchmark~\cite{TODO_iris} are relevant for two reasons. First, IRIS
...
Finally, SecLLMHolmes~\cite{TODO_secllmholmes} is a benchmark built
...
semantics-preserving edits --- renaming an identifier, inserting an
unrelated function call --- are sufficient to flip a correct verdict
to an incorrect one. This is the precedent for the four mutators of
